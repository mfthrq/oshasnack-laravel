            ...
            <div class="row g-5 align-items-xxl-center align-items-lg-start">
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 ml--30 mt_sm--50 mt_md--50 mt_lg--50 ml_sm--0">
                    <div class="team-details-wrapper">
                        <div class="d-flex">
                            <h3 class="title" data-sal-delay="300" data-sal-duration="800" data-sal="slide-up">Profile
                            </h3>
                        </div>
                        @if ($user->username)
                            <h6 class="fw-light">Halo, Selamat Datang <span class="fw-bold">
                                    {{ $user->username }}!</span></h6>
                        @endif
                    ...
                    <div class="d-flex gap-4">
                        ...
                        <button href="#" class="rts-btn btn-primary fw-bold" style="color: #771e56"
                            data-bs-toggle="modal" data-bs-target="#formModalEdit">Edit Data</button>
                        <form id="logout-form" action="{{ route('pelanggan.logout') }}" method="POST"
                            onsubmit="clearKeranjang(); event.preventDefault(); confirmLogout();">
                            @csrf
    ...
    <script>
        function confirmLogout() {
            Swal.fire({
                ...
                cancelButtonColor: '#CE3241',
                confirmButtonText: 'Ya, Logout!',
                cancelButtonText: 'Batal'
            }).then((result) => {
                if (result.isConfirmed) {
                    // Clear the keranjang (if needed)
                    clearKeranjang();
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="background-color: #771e56;">
            <div class="modal-header">
                ...
                <button style="background-color: #FEC10E;" type="button" class="me-4 btn-close" data-bs-dismiss="modal"
                    aria-label="Close">X</button>
            </div>
            <!-- Form untuk input data pelanggan -->
            <form class="login-wrapper w-100" id="editForm" method="POST"
                action="{{ route('profile-pelanggan.update', $user->id) }}">
                @csrf
                @method('PUT')
                <input type="hidden" value="{{ $user->id }}" name="id" id="editId">
                ...
                <button type="submit" class="fw-bold rts-btn btn-primary radious-5 w-100"
                    style="color: #771e56;">Perbarui</button>
            </form>
        </div>
    </div>
</div>
